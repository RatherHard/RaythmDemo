...
namespace Raythm::Audio
{
    ...
    struct AudioEngine::Impl
    {
        ...
        /** @brief Refreshes Ended state when playing time reaches the known track duration. */
        void refreshEndedState() noexcept
        {
            if (state != PlaybackState::Playing || duration <= AudioDuration::zero())
            {
                return;
            }
    ...
    bool AudioEngine::loadMusicFile(const std::string& filePath)
    {
        ...
        if (ma_sound_get_length_in_pcm_frames(m_impl->sound.get(), &lengthInFrames) == MA_SUCCESS)
        {
            const ma_uint32 sampleRate = ma_engine_get_sample_rate(&m_impl->engine);
            if (sampleRate > 0)
            {
                m_impl->duration = secondsToDuration(static_cast<double>(lengthInFrames) / static_cast<double>(sampleRate));
            }
        }
